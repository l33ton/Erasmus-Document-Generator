            ...
            if latest_version != current_version:
                def ask_user():
                    answer = messagebox.askyesno(
                        ...
                        f'Found version {latest_version} (current: {current_version}). Download?'
                    )
                    if answer:
                        assets = latest_release.get('assets', [])
                        dl_url = next((a['browser_download_url'] for a in assets if a['name'].endswith('.exe')), None)
                        cs_url = next((a['browser_download_url'] for a in assets if a['name'].endswith('.sha256')), None)
